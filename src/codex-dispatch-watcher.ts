...
function configuredDispatchDirs(): string[] {
  const configured = process.env.CODEMAN_CODEX_DISPATCH_DIRS;
  if (!configured) return [DEFAULT_DISPATCH_DIR];
  const dirs = configured
    .split(delimiter)
    .map((value) => value.trim())
    .filter(Boolean)
    .map((value) => (value === '~' ? homedir() : value.startsWith('~/') ? join(homedir(), value.slice(2)) : value));
  return dirs.length > 0 ? Array.from(new Set(dirs)) : [DEFAULT_DISPATCH_DIR];
}
...
export class CodexDispatchWatcher extends EventEmitter implements SubagentSource {
  ...
  getRecentSubagents(minutes: number = 60): SubagentInfo[] {
    const cutoff = this.now() - minutes * 60 * 1000;
    return this.getSubagents()
      .filter((agent) => agent.lastActivityAt >= cutoff)
  ...
  getStats(): SubagentWatcherStats {
    ...
    return {
      ...
      knownDirsCount: this.dispatchDirs.length + this.rolloutDirectories().length,
  ...
  private async scanNativeRollouts(): Promise<void> {
    const paths: string[] = [];
    for (const dir of this.rolloutDirectories()) {
      let entries;
      try {
        entries = await readdir(dir, { withFileTypes: true });
      } catch {
  ...
  private rolloutDirectories(): string[] {
    const directories = new Set<string>();
    for (let offset = -2; offset <= 1; offset++) {
      const date = new Date(this.now() + offset * 24 * 60 * 60 * 1000);
      ...
      for (const [year, month, day] of [local, utc]) {
        directories.add(
          join(this.codexSessionsDir, String(year), String(month).padStart(2, '0'), String(day).padStart(2, '0'))
        );
      }
    }
  ...
  private async processNativeRollout(filePath: string): Promise<void> {
    ...
    if (!cursor.agentId) {
      ...
      if (existing && this.agentFormats.get(agentId) === 'rollout') {
        ...
        const parent = scratchpadParent ?? directEvidence;
        if (!parent) return;
        const info: SubagentInfo = {
          ...
          description: directEvidence?.description ?? `codex exec (${basename(cursor.context.cwd) || 'worker'})`,
  ...
  private async readRolloutContext(filePath: string): Promise<CodexRolloutContext | undefined> {
    ...
    if (!threadId || !THREAD_ID_PATTERN.test(threadId) || !cwd || !isExec) return undefined;
  ...
  private async resolveRollout(threadId: string, startedAt: number): Promise<CodexRolloutContext | undefined> {
    ...
    for (let offset = -2; offset <= 2; offset++) {
      const date = new Date(startedAt + offset * 24 * 60 * 60 * 1000);
      const local = [date.getFullYear(), date.getMonth() + 1, date.getDate()];
      const utc = [date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate()];
      for (const [year, month, day] of [local, utc]) {
        candidates.add(
