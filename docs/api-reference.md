...
External integrations are registered service credentials, separate from
Codeman users. Registry management is admin-only:

- `POST /api/v1/integrations` with `{ id, label, owner?, eventTypes[] }` returns
  safe integration metadata plus the signing secret once.
- `GET /api/v1/integrations` lists metadata without secrets.
- `POST /api/v1/integrations/:id/rotate` returns replacement metadata plus a new
  secret once.
- `DELETE /api/v1/integrations/:id` revokes the integration.

The inbound webhook is:

```text
POST /api/v1/integrations/:id/events
X-Codeman-Timestamp: <unix seconds>
X-Codeman-Signature: v1=<hex hmac-sha256 of "timestamp.raw-body">
```

The JSON body is a versioned, typed event. The initial accepted types are
`job.updated`, `job.completed`, and `job.failed`; all three require an opaque
`boxm://job_...` subject. Event IDs are deduplicated per integration. A valid
new event returns `202`; an already accepted event returns `200` with
`duplicate: true`. Events outside the five-minute timestamp window or with an
invalid signature return `401`.

Accepted events are durably retained in a bounded inbox and update the current
job projection. Recovery reads are available through:

- `GET /api/v1/integrations/jobs`
- `GET /api/v1/integrations/events?limit=100`

The projection is broadcast through the stable SSE names
`integration:jobUpdated`, `integration:jobCompleted`, and
`integration:jobFailed`. Callers cannot choose arbitrary SSE event names.
