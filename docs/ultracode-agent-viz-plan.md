...
> **Status: IMPLEMENTED (2026-08-01, rev. 4) — Phases 1–4 shipped & verified.** A dedicated, opt-in **master-detail tab** (`showUltracodeAgents`, default OFF) shows ultracode/Workflow runs as Claude Code's "working agents" TUI: LEFT = runs + phases (selectable tasks), RIGHT = each run's agents with model, live state, **tokens burned**, and **tool calls**. Agent cards open tracked in-page transcript windows; active agents default to a bounded, follow-latest Live tail with an explicit Full mode.
>
...
> 3. **No per-agent transcript streaming was needed for the original v1.** The run-state JSON carries `tokens`/`toolCalls`/`state`/`label`/`phase` per agent. Rev. 4 later added the transcript link by reusing the already-tracked `agentId` and existing subagent route, without watcher changes.
